...
bool TargaImage::Quant_Populosity()
{
    ...
    {
        int number;
        number = (RGB_data[i] / 8) + (RGB_data[i + 1] * 4) + (RGB_data[i + 2] * 128);
        color[number].count++;
    }
    ...
    for (int i = 256; i < 32768; ++i)
    {
        ...
        for (int j = 0; j < 256; ++j)
        {
            distance = pow((double)color[i].r - (double)color[j].r, 2) +
    ...
    {
        int index = (RGB_data[k] / 8) + (RGB_data[k + 1] * 4) + (RGB_data[k + 2] * 128);
...
bool TargaImage::Dither_Bright()
{
    ...
    double average = 0.0, gray_data = 0.0;

    for (int i = 0; i < width * height * 4; i += 4)
        average += ((double)data[i] / (double)data[i + 3]);
    ...
    {
        gray_data = (double)data[i] / (double)data[i + 3];
        if (gray_data < average)
            data[i] = data[i + 1] = data[i + 2] = (double)0 * data[i + 3];
        else
            data[i] = data[i + 1] = data[i + 2] = (double)1 * data[i + 3];
    }

    cout << "brightness: " << average << endl;
    //ClearToBlack();
    return true;
}// Dither_Bright
...
{
   double mask[4][4] = { {0.7059, 0.3529, 0.5882, 0.2353},
   ...
   {
       gray_data = (double)data[i] / (double)data[i + 3];
       if (gray_data < mask[i % width % 4][i / width % 4])
           data[i] = data[i + 1] = data[i + 2] = (double) 0 * data[i + 3];
        else
            data[i] = data[i + 1] = data[i + 2] = (double) 1 * data[i + 3];
